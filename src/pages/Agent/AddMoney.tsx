...
const formSchema = z.object({
  ...
  userId: z.string().min(1, { message: "Select a user" }),
  amount: z
    .number()
...
export default function AddMoney() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const errorMessage = error?.data?.message || "Failed to process cash-in";
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Popover open={open} onOpenChange={setOpen}>
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
                        ...
                        role="combobox"
                        className={cn(
                          "w-full justify-between bg-white dark:bg-gray-700 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600",
                  ...
                  <PopoverContent className="w-full p-0 bg-white dark:bg-gray-700">
                    <div className="p-2">
                      <Input
                        placeholder="Search by phone or email..."
                        className="bg-white dark:bg-gray-600 text-gray-900 dark:text-white border-gray-300 dark:border-gray-500"
                        onChange={(e) => {
                    ...
                    {isSearching ? (
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {users.map((user) => (
                          <div
                            key={user._id}
                            className="flex items-center p-2 cursor-pointer hover:bg-accent dark:hover:bg-gray-600"
                            onClick={() => {
